...
mod tests {
    ...
    fn test_stack_long() {
        ...
        for &el in &source {
            assert!(stack.writer(&mut heap).write_all(&el.to_ne_bytes()).is_ok())
        }
        let mut buffer = Vec::new();
        stack.read_to_end(&heap, &mut buffer);
        let mut result = vec![];
        ...
        while !remaining.is_empty() {
            result.push(u16::from_ne_bytes(pop_2_bytes(remaining)));
    ...
    fn test_stack_interlaced() {
        ...
        for i in 0u32..9u32 {
            assert!(stack.writer(&mut heap).write_all(&i.to_ne_bytes() ).is_ok());
            vec1.extend_from_slice(&i.to_ne_bytes());
            if i % 2 == 0 {
                assert!(stack2
                    .writer(&mut heap)
                    .write_all(&i.to_ne_bytes())
                    .is_ok());
                vec2.extend_from_slice(&i.to_ne_bytes());
            }
        }
        let mut res1 = vec![];
        let mut res2 = vec![];
